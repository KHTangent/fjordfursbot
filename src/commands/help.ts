...
let newCommand: Command = {
	...
	execute(ctx) {
		...
		if (ctx.msg.guild && ctx.servers.has(ctx.msg.guild.id) && ctx.servers.get(ctx.msg.guild.id)!.modmailServerName) {
			helpMessage += `\`${ctx.botConfig.prefix}modmail ${ctx.servers.get(ctx.msg.guild.id)!.modmailServerName!} [message]\`` +
