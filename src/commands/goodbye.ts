...
let newCommand: Command = {
	...
	execute(ctx) {
		...
		if (ctx.msg.content.startsWith(`${ctx.botConfig.prefix}goodbye channel`)) {
			var id = ctx.msg.content.substring(`${ctx.botConfig.prefix}goodbye channel`.length+1).trim();
		...
		else if (ctx.msg.content.startsWith(`${ctx.botConfig.prefix}goodbye message`)) {
			var message = ctx.msg.content.substring(`${ctx.botConfig.prefix}goodbye message`.length+1).trim();
