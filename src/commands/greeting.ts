...
let newCommand: Command = {
	...
	execute(ctx) {
		...
		if (ctx.msg.content.startsWith(`${ctx.botConfig.prefix}greeting channel`)) {
			var id = ctx.msg.content.substring(`${ctx.botConfig.prefix}greeting channel`.length+1).trim();
		...
		else if (ctx.msg.content.startsWith(`${ctx.botConfig.prefix}greeting message`)) {
			var message = ctx.msg.content.substring(`${ctx.botConfig.prefix}greeting message`.length+1).trim();
